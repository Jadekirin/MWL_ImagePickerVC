...
@interface MWL_PhotoPreviewViewCell ()
...
@implementation MWL_PhotoPreviewViewCell
...
- (void)setModel:(MWL_PhotoModel *)model{
    _model = model;
    if (model.type == MWL_Photo) {
        _videoBgView.hidden = YES;
        _selectBtn.hidden = NO;
        _SB.hidden = NO;
    }else if (model.type == MWL_Video) {
    ...
    if (!model.image) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            UIImage *image = [UIImage imageWithCGImage:thumbnail scale:3.0 orientation:UIImageOrientationUp];
    ...
    if ([MWL_AssetManager sharedManager].selectedPhotos.count == 0) {
        model.ifSelect = NO;
    }
